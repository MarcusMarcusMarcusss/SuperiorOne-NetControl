...
界面采用任务管理器风格的浅灰背景、紧凑进程列表和黄色流量高亮。选中软件后点击右上角「结束任务」，确认后会结束该可执行文件路径下当前可访问的进程。未保存的内容可能丢失；已退出、受保护或无法确认路径的进程不会被结束。程序自身和系统关键进程受保护。历史记录保留，流量监控继续运行。列表刷新和排序不会改变已选中的软件。
...
本次任务管理器风格与「结束任务」更新已通过编译；新增的界面选择及进程终止测试已编写，但尚未执行验证。
